...
express()
  ...
  .get("/order", (req, res) => {
    ...
    let menu_info = { first: first_name, last: last_name, order: order };
  ...
  .post("/confirm", async (req, res) => {
    ...
    if (
      validateConfirm(first_name, last_name, street_address, city_state, order)
    ) {
      ...
      query_text +=
        ...
        "', '";
      query_text +=
        city_state + "', '" + order + "', now(), 'Received') RETURNING id;";
      ...
      try {
        ...
        const select_result = await client.query(
          "SELECT * FROM order_table WHERE id = " + order_number
        );
  ...
  .post("/service", async (req, res) => {
    try {
      ...
      const client = await pool.connect();
      const old_status_result = await client.query(
        "SELECT order_status FROM order_table WHERE id=" + order_number
      );
      ...
      // 'Delivered' -> 'Delivered'
      let new_status = "";
      if (old_status === "Received") new_status = "Cooking";
      else if (old_status === "Cooking") new_status = "Out For Delivery";
      else new_status = "Delivered";

      // update the db with the new status
      await client.query(
        "UPDATE order_table set order_status='" +
          new_status +
          "' where id=" +
          order_number
      );
...
function getSides(body) {
  ...
  if (body.item0 === "on") sides.push("Corn Bread");
  if (body.item1 === "on") sides.push("Creamed Corn");
  if (body.item2 === "on") sides.push("Green Beans");
  if (body.item3 === "on") sides.push("Mashed Potatos");
  if (body.item4 === "on") sides.push("Baked Beans");
